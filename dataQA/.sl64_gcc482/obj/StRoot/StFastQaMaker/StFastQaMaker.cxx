...
int StFastQaMaker::Init()
{
    ...
    mH_PVxy = new TH2D("h_PVxy","h_PVxy",200,-1,1,200,-1,1);

    mH_nPVvsBBCrate = new TH2D("h_nPVvsBBCrate","h_nPVvsBBCrate",900,200e3,1100e3,50,0,50);
    ...
    for (int i=0; i<2; ++i) { // 0,1:   1~ positive charged     0~ nagetive charged
        ...
        mH_tofBeta_ptr[i] = new TH2D(hn.Data(),hn.Data(),500,0,5,600,-1,5);
        hn = Form("h_tofBeta_gtr_%d", i);
        mH_tofBeta_gtr[i] = new TH2D(hn.Data(),hn.Data(),500,0,5,600,-1,5);
...
int StFastQaMaker::Make()
{
    ...
    mH_nPVvsBBCrate->Fill(bbcrate,npv);
    ...
    for (unsigned int iP=0; iP<nptr; ++iP) {
        ...
        double tP = tTrack->p().mag();
        if(tBtofM) mH_tofBeta_ptr[cF]->Fill(tP, 1.0/tBtofBeta);
        mH_bemcM_ptr[cF]->Fill(tBemcM);
        if(tBemcM) mH_bemcE_ptr[cF]->Fill(tP, tBemcE);
    }      
    for (unsigned int iG=0; iG<ngtr; ++iG) {
        ...
        double tP = tTrack->p().mag();
        mH_tofBeta_gtr[cF]->Fill(tP, 1.0/tBtofBeta);
        if(tBtofM) mH_bemcM_gtr[cF]->Fill(tBemcM);
        if(tBemcM) mH_bemcE_gtr[cF]->Fill(tP, tBemcE);
    }
    return kStOk;
}
