...
#include "StV0recMaker.h"


ClassImp(StV0recMaker)

StV0recMaker::StV0recMaker(const char *name, StMuDstMaker *muDstMaker, const char *out_fn)
: StMaker(name), m_MuDstMaker(muDstMaker), m_OutFn(out_fn) {}

void StV0recMaker::InitFile(void)
{
	cout << "StV0recMaker : output file: " << m_OutFn << endl;
	m_OutFile = new TFile(m_OutFn, "recreate");
}

void StV0recMaker::InitTree()
{
	m_OutTree = new TTree("V0rec_tree","Charged Track and V0 Reconstruction Information");
	...
	m_OutTree->Branch("v0event",&m_v0event);
}

void StV0recMaker::vClear(void)
{
	m_vptr_p.clear();
	m_vptr_n.clear();
	m_vgtr_p.clear();
...
bool StV0recMaker::checkTrack( StMuTrack *mt )
{
	short tFlag = mt->flag();
...
bool StV0recMaker::passV0( StV0event v0evt )
{
	if (v0evt.get_dca2()>2.0) return false;
	if (v0evt.get_cosrp()<0.0) return false;
	if (v0evt.get_p().Pt()<1.0) return false;
	if (v0evt.get_dcaV0()>2.0) return false;
	if (v0evt.get_dl()<0.5) return false;
}

int StV0recMaker::Init()
{
	InitFile();
	InitTree();
...
int StV0recMaker::Make()
{
	...
	if (m_MuDstMaker!=NULL)
	{
		...
		cout << "StV0recMaker : StMuDstMaker Error! " << endl;
		abort();
	}
	...
	{
		StMiniTrack gtrack_P = *iP;
		for (vector<StMiniTrack>::iterator iN = m_vgtr_n.begin(); iN != m_vgtr_n.end(); ++iN)
		{
			StMiniTrack gtrack_N = *iN;
			StV0event temp_V0( gtrack_P, gtrack_N, pv_position );
			if (!passV0(temp_V0)) continue;
			//if (jet_evt) temp_V0.associate_jet(jet_evt);
			m_v0event.push_back(temp_V0);
		}
	}
	m_v0event.shrink_to_fit();
